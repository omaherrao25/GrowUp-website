...
function WorkCard({ c }) {
  ...
  const [playing, setPlaying] = useState(false); // Start paused for lazy loading
  ...
  useEffect(() => {
    ...
    // Intersection Observer for lazy loading & play/pause
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        if (!isLoaded) {
          const source = vid.querySelector('source');
          if (source && source.dataset.src) {
            vid.src = source.dataset.src;
            vid.load();
            setIsLoaded(true);
          }
        }
  ...
  // Sync playing state with actual video playback
  useEffect(() => {
  ...
  return (
    <div className="work-card">
      ...
      {/* Link section omitted for brevity, same as original */}
